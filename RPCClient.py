...
def ArmPis():
    ...
    Clients=[RPCClient(url)]
    thread1 = threading.Thread(target=ArmPis_free, args=(Clients,[]))
    thread2 = threading.Thread(target=ArmPi_client, args=(url,))
    # 启动线程
    thread1.start()
    thread2.start()
    # thread2.start()        
...
#作为一个子线程运行
def ArmPi_client(url):
    # url='http://192.168.0.102:9030'
    Armclient = RPCClient(url)
    DBClient=HTTPClient('http://192.168.0.103:8000')
    ArmPi_catch(Armclient,DBClient,0)#机械臂组组号 0 1
